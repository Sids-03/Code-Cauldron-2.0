...
//    public static int upper_bound(long val,int start,long []arr){int low = start;int high = arr.length-1;int ans = -1;while(low<=high){int mid = (low+high)/2;if(arr[mid]<=val){low = mid + 1;ans  = mid;}else{high = mid - 1;}}return ans;}
  ...
  static void precomp(long Mod) {facts=new long[1_000_000];factInvs=new long[1_000_000];factInvs[0]=facts[0]=1;for (int i=1; i<facts.length; i++) facts[i]=mul(facts[i-1], i,Mod);factInvs[facts.length-1]=modInv(facts[facts.length-1],Mod);for (int i=facts.length-2; i>=0; i--) factInvs[i]=mul(factInvs[i+1], i+1,Mod);}
...
//        value+= dfs(n,m,(row+1),(col-1),arr);
  ...
  static class pair{ int a;int b;pair(int x,int y){this.a=x;this.b=y;} public int getFirst(){return (int) a;}public int getSecond() {return (int) b;}}
  public static class Tree<T> extends TreeSet<T> { public Tree() {super();}}

  static long[]vp;static ArrayList<Integer>[]lists;static int[] arr;static int[]brr;static char[] str;

  public static long stick(int node,int parent){return 0;}

  public static int recur(int n,int k,int[]arr){
    int prev=-1;ArrayList<Integer> list=new ArrayList<>();
    int[]fre=new int[n+1];
    TreeSet<Integer>[] ts=new TreeSet[n+1];
    for(int i=0;i<=n;i++){
      ts[i]=new TreeSet<>();ts[i].add((int) 1e9+i);
    }
    HashMap<Integer,Integer> map=new HashMap<>();
    for(int i=0;i<n;i++){
//                if(arr[i]==prev)continue ;
      list.add(arr[i]);prev=arr[i];
      if(!map.containsKey(arr[i])){
        map.put(arr[i],0);fre[arr[i]]=i+1;
      }
      ts[arr[i]].add(i+1);
    }
    int ans=0;PriorityQueue<pair> pq=new PriorityQueue<>(new Comparator<pair>() {
      @Override
    ...
    for(Integer e:list){
      while (pq.size()>0&&pq.peek().b!=fre[pq.peek().a])pq.poll();
      if(set.contains(e)){
        int l=ts[e].higher(fre[e]);
        fre[e]=l;
        pq.add(new pair(e,fre[e]));continue;
      }
      if(set.size()==k){
        pair d=pq.poll();
        set.remove(d.a);
      }
      set.add(e);int l=ts[e].higher(fre[e]);fre[e]=l;
      pq.add(new pair(e,fre[e]));
      pair r=pq.peek();
      ans++;
    }
    return ans;
  ...
  public static int recur1(int n,int[]t,int k){
    boolean[] have = new boolean[500500];
    int res = 0;
    int[] comp = compress(t);
    int[] next = new int[comp.length];
    int[] last = new int[500500];
    Arrays.fill(last, 1000000);
    ...
    res = 0;
    int cap = 0;
    have = new boolean[500500];
    for(int i = 0; i < comp.length; i++) {
      while (!pq.isEmpty() && pq.peek().pos < i) pq.poll();
      if(cap < k) {
        if(!have[comp[i]]) {
          have[comp[i]] = true;
          res++;
        ...
        if(!have[comp[i]]) {
          ...
          res++;
          if (!pq.isEmpty()) {
            have[pq.poll().num] = false;
          }
        }
      }
      pq.add(new Obj(comp[i], next[i]));
    }
    return res;
  }
...
//      precomp(MoD);
    ...
    superOuter:
    while(Te-- >0){
      int n=s.nextInt(),T=s.nextInt();int[]arr=s.readIntArray(n,0,n);
      HashMap<Integer,Integer> map=new HashMap<>();int cou=1;
      for(int i=0;i<n;i++){
        if(map.containsKey(arr[i]))continue ;
        map.put(arr[i],cou++);
      }
      for(int i=0;i<n;i++)arr[i]=map.getOrDefault(arr[i],0);
//      for(int i=1;i<=n;i++){
//        int l=recur1(n,arr,i);
//        System.out.println(i+" "+l);
//      }
      int l=1,r=n;int ans=-1;
      while (l<=r){
        int mid=(l+r)/2;
        int d=recur1(n,arr,mid);
        if(d>T){
          l=mid+1;
        }else{
          if(d<=T)ans=mid;
