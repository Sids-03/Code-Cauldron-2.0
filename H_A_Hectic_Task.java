...
//      Reader s = new Reader();
      precomp(MoD);
		...
		superOuter:
		while(Te-- >0){
			int n=s.nextInt();str=s.next();
			int[]arr=new int[26];for(int i=0;i<n;i++) arr[str[i]-'a']++;
			n=26;
			long inv=1;int sum=0;
			for(int i=0;i<n;i++){sum+=arr[i];
				inv*=factInvs[arr[i]];inv%=MoD;
			}
			long ans=0;
			for(int i=0;i<n;i++){
				long d=facts[sum-arr[i]];d*=inv;d%=MoD;d*=facts[arr[i]];d%=MoD;
				ans+=d;ans%=MoD;
			}
